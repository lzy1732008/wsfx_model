...
class TextRNN(object):
    ...
    def rnn(self):
        ...
        with tf.name_scope("attention"):
            ...
            #对行做attention
            beta = tf.reduce_sum(tf.nn.softmax(dot,axis=2),axis=1)
            newbeta = tf.expand_dims(beta,axis=1)
            alpha = tf.nn.softmax(dot,axis=1)
            result = tf.matmul(alpha,newbeta,adjoint_b=True)


        with tf.name_scope("score"):
            # 全连接层，后面接dropout以及relu激活
            fc = tf.layers.dense(result[:,:,0], self.config.hidden_dim, name='fc1')#w*input+b,其中可以在此方法中指定w,b的初始值，或者通过tf.get_varable指定
