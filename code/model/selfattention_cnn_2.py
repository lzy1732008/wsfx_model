    ...
    def cnn(self):
        with tf.name_scope("attention1"):
             with tf.variable_scope("linear1"):
                 ...
                 w1 = tf.Variable(tf.random_normal([2, 2, 1 , 1], stddev=2, seed=1),
                                  trainable=True, name='w1')
                 w2 = tf.Variable(tf.random_normal([2, 2, 1, 1], stddev=2, seed=2),
                                  trainable=True, name='w2')
                 w3 = tf.Variable(tf.random_normal([2, 2, 1, 1], stddev=2, seed=3),
                                  trainable=True, name='w2')
                 Q = tf.nn.conv2d(newinput1, w1, strides=[1, 1, 1, 1], padding='SAME')[:,:,:,0]
                 K = tf.nn.conv2d(newinput1, w2, strides=[1, 1, 1, 1], padding='SAME')[:,:,:,0]
                 V = tf.nn.conv2d(newinput1, w3, strides=[1, 1, 1, 1], padding='SAME')[:,:,:,0]
             ...
             with tf.variable_scope("selfattention1"):
                 ...
                 t3_1 = tf.matmul(t2, V)#[None,seq1,dim]
        ...
        with tf.name_scope("attention2"):
             ...
             with tf.variable_scope("selfattention2"):
                 t1 = tf.matmul(Q,K,adjoint_b=True)/math.sqrt(self.config.embedding_dim)       #[None,seq1,dim]*[None,dim,seq2] = [None.seq1,seq2]
                 t2 = tf.nn.softmax(t1, axis=2)
                 t3_2 = tf.matmul(t2, V)#[None,seq2,dim]

        with tf.name_scope("attention3"):
            p = tf.matmul(t3_1,t3_2,adjoint_b=True)
        ...
        with tf.name_scope("cnn3"):
            ...
            with tf.variable_scope("cnn-var3"):
                conv = tf.layers.conv1d(p, self.config.num_filters, self.config.kernel_size,
                                        name='conv')  # (?,26,256)
                # global max pooling layer
